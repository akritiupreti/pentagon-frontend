...
export async function submitIntervention(
  ...
  const res = await fetch(
    `${BASE_URL}/sessions/${sessionId}/intervention/${sampleId}`,
    {
      method: "POST",
      headers: authHeaders(),
...
export async function updateSessionStatus(sessionId: string, status: string) {
  const res = await fetch(
    `${BASE_URL}/sessions/${sessionId}/status?status=${status}`,
    {
      method: "PATCH",
      headers: authHeaders(),
    },
...
export async function uploadDataset(sessionId: string, imageCount: number) {
  const res = await fetch(
    `${BASE_URL}/sessions/${sessionId}/upload?image_count=${imageCount}`,
    {
      method: "POST",
      headers: authHeaders(),
    },
...
export async function startInferencing(payload: {
  keys: string[];
  modelType: string;
  userId: string;
  sessionId: string;
