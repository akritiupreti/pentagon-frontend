    ...
    function connectWebSocket(token: string) {
        ...
        ws.onmessage = (event) => {
            ...
            if (msg.type === "metric") {
                ...
                setDecisions((prev) => [...prev, { message: msg.data.message, timestamp: new Date().toISOString() }])
            }
        }
    }
    ...
    return (
        <main className="min-h-screen bg-black text-white flex flex-col">
            <nav className="flex items-center justify-between px-10 py-6 border-b border-white/10">
                <Link to="/" className="text-xl font-black tracking-[0.3em] text-white">PENTAGON</Link>
                <Link to="/dashboard" className="text-white/40 text-sm hover:text-white transition-colors">Back to Dashboard</Link>
            </nav>

            <div className="flex-1 px-10 py-12 max-w-6xl mx-auto w-full">
                <div className="flex items-start justify-between mb-10">
                    <div>
                        <div className="text-white/30 text-xs tracking-widest uppercase mb-1">Session Detail</div>
                        <h1 className="text-4xl font-black tracking-tight mb-2">{session.name}</h1>
                        <div className="text-white/40 text-sm">{session.architecture} · {session.task}</div>
                    </div>
                    <span className={`text-xs px-3 py-1.5 border ${statusColors[session.status] || "text-white/40 border-white/10"}`}>{session.status}</span>
                </div>

                <div className="border border-white/10 p-6 mb-6">
                    <div className="text-white/30 text-xs tracking-widest uppercase mb-3">API Key</div>
                    {apiKey ? (
                        <div className="flex items-center gap-4">
                            <code className="flex-1 text-sm text-white/70 bg-white/5 px-4 py-2 font-mono">{keyVisible ? apiKey : "••••••••••••••••••••••••••••••••"}</code>
                            <button
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
                    <div className="border border-white/10 p-6">
                        <div className="text-white/30 text-xs tracking-widest uppercase mb-6">Loss Curve</div>
                        {metrics.length === 0 ? (
                            <div className="h-48 flex items-center justify-center text-white/20 text-sm">Waiting for training data...</div>
                        ) : (
                            <ResponsiveContainer width="100%" height={200}>
                                <LineChart data={metrics}>
                                    <XAxis dataKey="epoch" stroke="#ffffff20" tick={{ fill: "#ffffff40", fontSize: 11 }} />
                                    ...
                                    />
                                    <Line type="monotone" dataKey="loss" stroke="#ffffff" strokeWidth={2} dot={false} />
                                </LineChart>
                            </ResponsiveContainer>
                        )}
                    </div>
                    ...
                    <div className="border border-white/10 p-6">
                        <div className="text-white/30 text-xs tracking-widest uppercase mb-6">Accuracy</div>
                        {metrics.length === 0 ? (
                            <div className="h-48 flex items-center justify-center text-white/20 text-sm">Waiting for training data...</div>
                        ) : (
                            <ResponsiveContainer width="100%" height={200}>
                                <LineChart data={metrics}>
                                    ...
                                    />
                                    <Line type="monotone" dataKey="accuracy" stroke="#4ade80" strokeWidth={2} dot={false} />
                                </LineChart>
                            </ResponsiveContainer>
                        )}
                    </div>
                ...
                <div className="border border-white/10 p-6 mb-6">
                    <div className="text-white/30 text-xs tracking-widest uppercase mb-4">Agent Decision Feed</div>
                    <div className="h-48 overflow-y-auto flex flex-col gap-2">
                        {decisions.length === 0 ? (
                            <div className="flex items-center justify-center h-full text-white/20 text-sm">Waiting for agent decisions...</div>
                        ) : (
                            decisions.map((d, i) => (
                                <div key={i} className="flex items-start gap-4 text-sm">
                                    <span className="text-white/20 text-xs font-mono shrink-0">{new Date(d.timestamp).toLocaleTimeString()}</span>
                ...
                <div className="border border-yellow-400/20 bg-yellow-400/5 p-6 flex items-center justify-between">
                    <div>
                        <div className="text-yellow-400 text-xs tracking-widest uppercase mb-1">Human Intervention</div>
                        <p className="text-white/50 text-sm">Review samples that the AI agent has escalated for human labeling.</p>
                    </div>
                    <Link
